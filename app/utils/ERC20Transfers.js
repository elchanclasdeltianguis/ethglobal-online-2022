...
const pruneTransfers = (transfersData, address) => {
  ...
  const transfers = transfersData.map((transfer) => {
    ...
    const to =
      transferFlow === "Out"
        ? transferLogEvent[0].decoded.params[1].value
        : address; //Notes on this field: this specifies where the funds are transferred to. Not the contract interacted with.
    ...
    const transferValue =
      parseInt(transferLogEvent[0].decoded.params[2].value) /
      10 ** parseInt(transferLogEvent[0].sender_contract_decimals);
    const tokenAddress = transferLogEvent[0].sender_address;
    const txnHash = transfer.tx_hash;
    const isERC721 =
      transferLogEvent[0].decoded.params[2].name == "tokenId" ? true : false;
    ...
    if (isMultipleTransfers) {
      multipleTransfers = innerTransfersLogItem.map((transfersItem) => {
        ...
        const amount =
          parseInt(transfersItem.decoded.params[2].value) /
          10 ** parseInt(transferLogEvent[0].sender_contract_decimals);
    ...
    multipleTransfers.sort(
      (a, b) => parseFloat(a.logOffset) - parseFloat(b.logOffset)
  ...
  const transfersWithoutNFTs = transfers.filter(
    (transfer) => !transfer.isERC721
  );
